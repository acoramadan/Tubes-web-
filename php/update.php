...
if(isset($_SESSION['form_data'])){
    ...
    if(isset($data['stambuk'])){
        ...
        $result = mysqli_query($con, "SELECT * FROM pendaftaran WHERE stambuk='$stambuk'");
        if(mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_array($result);
        } else {
            echo "Error: Data not found for stambuk '$stambuk'.";
            exit;
        }
    } else {
        echo "Error: Stambuk data missing in session.";
        exit;
    }
} else {
...
<div class="container form-container">
    <form action="sesup.php" method="post" enctype="multipart/form-data">
        <h2 class="text-center mb-4">Form Pendaftaran CA</h2>
        <div class="mb-3">
            <label for="nama" class="form-label">Nama Lengkap</label>
            <input type="text" class="form-control" id="nama" name="nama" placeholder="Nama Lengkap" value="<?php echo isset($row['nama']) ? $row['nama'] : ''; ?>" required>
        </div>
        <div class="mb-3">
            <label for="stambuk" class="form-label">Stambuk</label>
            <input type="number" class="form-control" id="stambuk" name="stambuk" placeholder="Stambuk" value="<?php echo $row['stambuk']; ?>" required>
        </div>
        <div>
            <label>Jurusan</label>
        ...
        <div class="mb-3">
            ...
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="gender" id="inlineRadio1" value="perempuan" <?php echo $row['jenis_kelamin'] == 'perempuan' ? 'checked' : ''; ?> required onclick="updateKelasOptions()">
                <label class="form-check-label" for="inlineRadio1">Perempuan</label>
            </div>
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="gender" id="inlineRadio2" value="laki-laki" <?php echo $row['jenis_kelamin'] == 'laki-laki' ? 'checked' : ''; ?> required onclick="updateKelasOptions()">
        ...
        <div class="mb-3 form-floating">
            <textarea class="form-control" placeholder="Alasan mendaftar" id="floatingTextarea2" name="alasan" style="height: 150px" required><?php echo $row['alasan']; ?></textarea>
...
<script>
    function updateKelasOptions() {
        ...
        if (gender === 'perempuan') {
            ...
            kelasOptions = ['A1', 'A2', 'A3', 'A4', 'A5', 'A6'];
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        updateKelasOptions();
        const kelasValue = "<?php echo $row['kelas']; ?>";
        document.getElementById('floatingSelect').value = kelasValue;
    });
</script>
</body>
</html>